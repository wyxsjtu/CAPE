"""
block_sweep_peer0_mlpcls.py

CAPE on ASCADv1 using an MLP
classification model.  Three-phase pipeline:

  Phase 1 — Train NUM_BLOCKS×256 MLPCLS models (one per block×key guesses).
...
Model architecture (MLPCLS, classification):
  Linear(700, HIDDEN1) + BN + ReLU → Linear(HIDDEN1, HIDDEN2) + BN + ReLU
  → Linear(HIDDEN2, NUM_CLASSES)
  Label modes:
    'HW'  — Hamming weight of SBOX[p⊕k],  NUM_CLASSES=9  (HW ∈ {0,...,8})
...
MODEL_DIR = os.path.join(_BASE_DIR, f'models_mlpcls_{LABEL_MODEL.lower()}_{NUM_TRACES}')
FIG_DIR   = os.path.join(_BASE_DIR, f'figures_mlpcls_{LABEL_MODEL.lower()}_{NUM_TRACES}')
...
class MLPCLS(nn.Module):
    ...
    def forward(self, x):
        # x: (B, SEG_LEN) → logits: (B, NUM_CLASSES)
        return self.head(self.hidden(x))

    def extract_features(self, x):
        """Return first hidden layer output (after BN+ReLU) for MMD computation."""
        return F.relu(self.hidden[1](self.hidden[0](x)))   # (B, HIDDEN1)
...
def _p1_worker(task_queue, result_queue, gpu_id,
    ...
    while True:
        ...
        # Ranked by final training CE loss; no validation set needed for Phase 1.
...
def main():
    ...
    print(f'Model: MLPCLS  LABEL_MODEL={LABEL_MODEL}  SEG_LEN={SEG_LEN}  '
          f'HIDDEN1={HIDDEN1}  HIDDEN2={HIDDEN2}  NUM_CLASSES={NUM_CLASSES}')
